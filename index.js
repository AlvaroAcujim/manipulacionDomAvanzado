...
const itemsList = document.getElementsByClassName(
  "main__container__section__buttonsAction__button__itemsContainer__items"
);
const itemsParagraph = document.getElementsByClassName(
  "main__container__section__buttonsAction__button__itemsContainer__items__paragraph"
);
    ...
    el.addEventListener("click", (ev) => {
      ...
      if (itemSelected) {
        ...
        const txt = document.createTextNode(
          "Ha seleccionado el elemento: " + ev.target.children[0].textContent
        );
        ...
        for (let el of itemsList) {
          let txt =
            "main__container__section__buttonsAction__button__itemsContainer__items";
          txt =
            el.getAttribute("id") === itemSelected ? txt + " selected" : txt;
...
arrayButtons.forEach((el) => {
  el.addEventListener("click", () => {
    if (valueInput) {
      //comprueba que tenga valor el input
      switch (el) {
        case create:
        ...
        case addBefore:
          itemSelected
            ...
            : alert("Seleccione un div"); //si el usuario pulsa fuera de los divs no se recoge el id por lo que se le solicita en caso de que no encuentre el elemento que lo seleccione.
          break;
        case addAfter:
          itemSelected
            ? addAfterItem(itemSelected, valueInput)
            : alert("Seleccione un div");
          break;
        case replace:
          itemSelected
...
remove.addEventListener("click", () =>
  itemSelected ? deleteItem(itemSelected) : alert("Seleccione un div")
);
